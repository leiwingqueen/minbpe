...
PS：karpathy的仓库本身就附带了实现的源码。有兴趣的实现的同学也可以clone我的仓库，里面exercise下的分支已经包含了带中文注释的练习骨架 —— 核心方法被挖空，注释里写清了每一步该做什么和会踩什么坑。
...
其实完全可以。最朴素的语言模型就是在原始字节上做自回归：用前 k 个 byte 预测下一个 byte。词表固定 256，永远没有未登录词，连分词器这个模块都不需要 —— karpathy 在课程里就是从这条路讲起的，早期的实验也确实这么干。

问题出在序列长度上。同一篇文章，按字节算是 185,768，按 token 算只有 45,338（本仓库实测），差 4.10 倍。而 attention 是 `O(n²)`：序列长 4 倍，注意力的计算量就是 16 倍。上下文窗口、显存、训练成本，全都被消耗在「拼字母」这件事上。

> **所以 tokenizer 本质上是一层压缩。** 把语料里高频出现的字节组合压成一个 id，用**词表大小**去换**序列长度**。256 → 512 个 token 的代价，换来的是序列缩短一半。

第二个作用同样重要，甚至更本质：**压缩顺便给了模型一个「有意义的单位」。** 在字节层面，`the` 是 116、104、101 三个互不相干的整数，模型得自己从海量数据里学出「这三个数经常连在一起，且连在一起时表示一个冠词」。而合并成一个 token 之后，`the` 在 embedding 矩阵里直接就是一个独立的向量，模型从第一步起拿到的就是一个语义单元，而不是三个字符碎片。

这也解释了后面第 07 节为什么要费劲用正则先划边界 —— 我们希望压出来的单位尽量贴近**语言本身的单位**，而不只是统计意义上的高频字节串。压缩率和语义质量之间是要做取舍的，BPE 的目标从来不是最优压缩。

当然，词表也不是越大越好：词表越大，embedding 矩阵和输出层 softmax 越大，每个 token 分到的训练样本越稀疏。词表大小是这条曲线上的一个折中点。
